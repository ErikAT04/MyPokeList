...
myList = {}  # Set que muestra los pokémon que hay guardados en la lista personalizada
"""
La lista se guarda en un set porque es una lista no ordenable que no permite valores repetidos, 
cosa que nos interesa en la lista si hay algún error o se edita el archivo de texto a voluntad para meter dos pokémon iguales y romper el código"""
...
def initLists():  # Función que se inicia al principio del programa: Inicia las listas y prepara el archivo de la lista personalizada
    pokeList.append(Pokemon(0, "Missingno", 0, 0, [{"type": {"name": "null"}}]))  # Introduzco a MissingNo, un pokémon "no oficial" que no aparece en el api
    ...
    print("Cargando datos, espere al nombre del último pokémon:")  # Más bien orientativo, para que el usuario sepa por dónde llega la carga
    while statuscode == 200: #Para empezar: Mientras se encuentren siempre datos (El código de estado sea 200):
        pokeSearch = requests.get("https://pokeapi.co/api/v2/pokemon/" + str(i))  # Hace una búsqueda en el pokémon cuyo número de la pokedex es el iterador
        ...
        if statuscode == 200:  # Si es 200 (se ha realizado correctamente la conexión)
            ...
            pokemon: Pokemon = Pokemon(jsonSearch["id"], jsonSearch["name"], jsonSearch["height"], jsonSearch["weight"],
                                       jsonSearch["types"])  # Guarda los datos que nos interesan en un objeto de tipo Pokemon
    ...
    pokeSearch = requests.get("https://pokeapi.co/api/v2/type/")  # Hace ahora una búsqueda de los tipos registrados en el api
    ...
    try:  # Inicio del control de excepciones
        with open("regionlist.json", "r") as regionListFile: # Se abre el archivo json de la lista de regiones (La API no daba la información que buscaba)
        ...
        with open(myPokeFile, "r") as myFile:  # Se abre el archivo personalizado en modo lectura
            ...
            for line in myFile:  # Por cada linea en el archivo, guarda el nombre en la lista
                listAux.append(line[0:len(line) - 1])  # Quito el salto de linea (\n) que tiene el fichero guardado en cada uno
...
def addToList():  # Función encargada de guardar en la lista personalizada el pokémon que nos interese
    ...
    if not isAlready:  # Si en este momento la booleana sigue siendo true (no ha aparecido el pokémon en la lista):
        for pokemon in pokeList: # Recorre esta vez la lista de pokemon general en un forEach
            if (pokemonIdent.isdigit() and int(pokemonIdent) == pokemon.dexId) or pokemonIdent.lower() == pokemon.name: # Si el nombre o id cumple la condición de antes:
...
def allPokeNames() -> str:  # Función que muestra los nombres de todos los pokémon del api
    ...
    for i in range(len(pokeList)):  # Recorre la lista entera de pokemon
        allNames = allNames + pokeList[i].name  # Añade al final del string el nombre del pokémon de la instancia en ese momento
        if i < len(pokeList) - 1:  # Por estética, si el iterador no ha llegado al final de la lista, añade una coma y un espacio
            allNames = allNames + ", "
...
def seeOneRegion():  # Función que muestra todos los pokémon de la lista del api de la región que diga el usuario
    reg = input("Escribe el nombre o id de la región (1 = Kanto, 2 = Johto...): ")  # Guarda el id o el nombre de la región
    ...
    for region in regionList:  # Recorre la lista de regiones para verificar si la región puesta existe
        if (reg.isdigit() and int(reg) == region.id) or (reg.lower() == region.name):  # Si el valor introducido es de digitos y es igual al id, o si es igual al nombre de la región (minúsculas):
            regionFound = True  # La booleana se vuelve true
            print(f"Mostrando pokédex de {region.name.capitalize()}:")
            for i in range(region.firstPoke, (region.lastPoke + 1)):  # Entra en el rango del primer y último pokémon de la región e imprime todos los pokémon de la lista
...
def quitFromList():  # Función que borra los pokémon de la lista puesta
    ...
    for pokemon in myList:  # Recorre la lista personalizada
        if (poke.isdigit() and pokemon.dexId == int(poke)) or (poke.isalpha() and poke.lower() == pokemon.name):  # Si el id (solo digitos) o el nombre (minúsculas) coincide con el de algún pokémon:
            ...
            with open(myPokeFile, "w") as myFile:  # Abre el fichero de la lista en modo escritura (borra lo que hubiera en el fichero previamente)
...
def filter(fun, passedList) -> []:  # Función DE ORDEN SUPERIOR que devuelve una lista filtrada según qué función reciba de parámetro
        ...
        try: # Abre el control de excepciones
            ...
            match opt:  # Switch en función de la opción
                case 1:  # Si se ha elegido filtrar por tipo:
                    ...
                    for t in listTypes:  # Recorre la lista de tipos
                        if pokeType.lower() == t.lower():  # Si coincide que el tipo escrito existe en la lista:
                            ...
                            filteredPokeList = filter(lambda x: t in x.types, myList)  # Filtra con una función lambda que solo es true si el tipo escrito se encuentra entre los tipos del pokémon
                ...
                case 2:  # Si se ha elegido filtrar por rango de valores en la pokédex:
                    ...
                    if num1 < 0 or num2 < 0 or num1 > len(pokeList) or num2 > len(pokeList): # Si alguno de ellos se sale del rango de la lista avisa al usuaro
                        ...
                        if num1 < num2:  # Si el primer número es menor que el segundo, sigue la función correctamente
                            print(f"Preparando lista entre los valores {num1} y {num2}: ")  # Avisa al usuario de que va a filtrar
                            filteredPokeList = filter(lambda x: num1 <= x.dexId <= num2, myList)  # Filtra con una función lambda que solo es true si el número de la pokédex se encuentra entre num1 y num2 (incluidos)
                ...
                case 3:  # Si el usuario ha elegido filtrar por caracteres en el nombre:
                    regex = input("Escribe los caracteres que debe tener el nombre de los pokémon: ")  # Guarda la cadena de caracteres
                    filteredPokeList = filter(lambda x: regex.lower() in x.name.lower(), myList)  # Filtra con una función lambda que solo es true si el nombre contiene la cadena de caracteres dada
...
def seeMyListInRegions():  # Función muestra los pokémon de la lista personalizada en función de la región a la que pertenecen
    for region in regionList:  # Por cada región registrada:
        ...
        for pokemon in myList: # Por cada pokémon de la lista personalizada:
            if pokemon.dexId in range(region.firstPoke, region.lastPoke+1):  # Si el pokémon se encuentra en el rango de la pokédex de la región, se imprime el pokémon
                print(pokemon)
